import { Box, Card, CardActionArea, CardContent, CardMedia, Chip, Container, FormControl, Grid, InputLabel, MenuItem, Select, Typography } from "@mui/material";
...
const CategoriesPage: React.FC<CategoriesPageProps> = ({ categorySearch }) => {
	...
	useEffect(() => {
		if (searchCategory && categories.size > 0) {
			...
			const matchingCategory = Array.from(categories.values()).find((cat) => normalize(cat.name) === normalize(searchCategory));
		...
		<Box sx={{ background: "#fafafa", minHeight: "100vh", pb: 4 }}>
			...
			<Box sx={{ display: "flex", justifyContent: "center", mb: 6 }}>
				<FormControl sx={{ minWidth: 200, background: "#fff", borderRadius: 3, boxShadow: "0 2px 8px rgba(0,0,0,0.1)" }}>
					...
					<Select labelId="category-select-label" value={selectedCategory} onChange={handleCategoryChange} label="Theme" sx={{ borderRadius: 3 }}>
