...
const PhotoPurchaseEdit: React.FC<PhotoPurchaseEditProps> = ({ open, photo, onClose }) => {
  ...
  const handleSave = async () => {
    ...
    if (response.ok) {
      ...
      photo.purchase_now_price = Number(editedPrice);
      onClose();
    } else {
  ...
  return (
    ...
    >
      <DialogTitle sx={{ display: "flex", alignItems: "center", gap: 1, color: "#fff" }}>
        ...
        <IconButton onClick={onClose} sx={{ color: "#fff" }}>X</IconButton>
      </DialogTitle>
      ...
      <DialogContent>
        ...
        >
          <img
            src={photo.file_path}
            alt={`Photo ${photo.id}`}
            style={{ width: "100%", marginBottom: "20px", borderRadius: "8px", boxShadow: "0 4px 8px rgba(0,0,0,0.2)" }}
          />
