...
const ValidatorPanel: React.FC = () => {
  ...
  return (
    <Box sx={{ minHeight: "100vh", padding: "40px 20px", background: "linear-gradient(135deg, #c3ec52 0%, #0ba29d 100%)" }}>
          ...
          <Grid item xs={12} sm={6} md={4} lg={3} key={item.id}>
            <Paper elevation={6} sx={{ borderRadius: 3, overflow: "hidden" }}>
              <Card sx={{ borderRadius: 0, boxShadow: "none" }}>
                <CardMedia component="img" height="200" image={item.photoUrl} alt="Photo" sx={{ objectFit: "cover" }} />
                <CardContent>
                  <Box display="flex" alignItems="center" mb={1} gap={1}>
                    <LocationOnIcon color="error" />
                    <Typography variant="body1" fontWeight="bold">
                      <a href={item.locationUrl} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none', color: 'inherit' }}>
                  ...
                  <Box display="flex" alignItems="center" gap={1} mb={1}>
                    ...
                    <Typography variant="body2" fontWeight="bold">Photo Status:</Typography>
                    <Chip label={item.photoStatus.toUpperCase()} color="warning" size="small" sx={{ fontWeight: "bold" }} />
                  </Box>
                  <Box display="flex" alignItems="center" gap={1} mb={1}>
                    ...
                    <TextField
                      ...
                      sx={{ flex: 1 }}
                    />
                    <Button variant="contained" color="info" size="small" onClick={() => handleSaveCategory(item.id)}>
                      Save
                    </Button>
                  </Box>
                </CardContent>
                <CardActions sx={{ display: "flex", justifyContent: "center", gap: 2 }}>
                  <Button variant="contained" color="success" startIcon={<CheckIcon />} onClick={() => handleApprove(item.id)}>Approve Photo</Button>
                  <Button variant="contained" color="error" startIcon={<ClearIcon />} onClick={() => handleReject(item.id)}>Reject Photo</Button>
                </CardActions>
              </Card>
            </Paper>
          </Grid>
